...
function greedyPartition(players, weightKey, multiplier = 1.0) {
    const sorted = fisherYates(players).sort((a, b) => (b[weightKey] * multiplier) - (a[weightKey] * multiplier));
    const teamA = [];
    const teamB = [];
    let sumA = 0, sumB = 0;
...
export function balanceRandom(players, teamSize) {
    ...
    const assign = (team, offset) => team.map((p, i) => ({ ...p, assigned_role: roles[i + offset] }));
    return [assign(a, 0), assign(b, a.length)];
}
export function balanceWeightedWithRoles(players, teamSize) {
    const cfg = ROLE_CONFIGS[teamSize];
    const needT = cfg.T * 2;
