...
export function exportCSV(players: Player[]): string {
  const header = 'name,t,d,s,locked_role';
  const rows = players.map(p =>
    `${escapeCsv(p.name)},${p.t},${p.d},${p.s},${p.locked_role || ''}`
  );
...
export function parseCSV(text: string): Player[] {
  ...
  if (lines.length < 2) throw new Error('CSV 为空或只有表头');
  ...
  if (nameIdx < 0 || tIdx < 0) throw new Error('CSV 格式错误：需要 name,t,d,s,locked_role');

  const players: Player[] = [];
  for (let i = 1; i < lines.length; i++) {
    const cols = parseCSVLine(lines[i]);
    const locked = lockIdx >= 0 ? cols[lockIdx]?.trim() || '' : '';
    const lockedRole: RoleKey | null = locked === 'T' ? 'T' : locked === 'DPS' ? 'DPS' : locked === 'S' ? 'S' : null;
